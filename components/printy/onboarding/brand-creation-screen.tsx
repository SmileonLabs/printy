...
export function BrandCreationScreen() {
  const { brandDraft, brandLogoSetupMode, updateBrandDraft, setStep } = usePrintyStore();
  const canContinue = brandDraft.name.trim().length > 0 && brandDraft.category.trim().length > 0;
  const nextStep = brandLogoSetupMode === "upload" ? "logoUpload" : "logoDirection";

  return (
    <Screen footer={<AppButton onClick={() => setStep(nextStep)} disabled={!canContinue} className="disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0">다음 단계로</AppButton>}>
