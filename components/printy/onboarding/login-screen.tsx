...
export function LoginScreen() {
  ...
  return (
    <Screen>
      <ProgressHeader eyebrow="간편 가입" title={isCheckoutLogin ? "주문을 저장할 계정을 만들어요" : "브랜드 저장을 마무리해요"} description={isCheckoutLogin ? "처음이면 자동 가입되고, 이미 가입한 정보면 바로 로그인됩니다." : "저장한 로고와 브랜드를 My Brand에서 바로 확인할 수 있어요."} step={stepNumbers.login} total={onboardingTotalSteps} action={<HomeExitAction />} />
      <form className="grid gap-4 pt-2" onSubmit={handleSubmit}>
        ...
        <div className="grid gap-3">
          <a className="block rounded-md bg-ink px-5 py-4 text-center text-base font-extrabold text-white shadow-soft transition duration-200 hover:-translate-y-0.5" href={`/api/auth/google/start?redirect=${redirectTarget}`}>
            Google로 계속하기
          </a>
          <a className="block rounded-md bg-[#fee500] px-5 py-4 text-center text-base font-extrabold text-[#191919] shadow-soft transition duration-200 hover:-translate-y-0.5" href={`/api/auth/kakao/start?redirect=${redirectTarget}`}>
            Kakao로 계속하기
          </a>
        </div>
      </form>
