...
import { CompletedDesignCard, completedBusinessCardMockupImageSize, completedDesignPreviewClassName, completedDesignPreviewImageSizes } from "@/components/design-production/completed-design-card";
      ...
      <div className="columns-2 gap-3 sm:columns-3 xl:columns-4">
        {entries.map((entry) => {
          ...
          return (
            <CompletedDesignCard
              key={mockup.id}
              preview={mockup.cleanImageUrl && completedLayout && matchedMember ? <BusinessCardUserPreview className={completedDesignPreviewClassName} cleanImageUrl={mockup.cleanImageUrl} layout={completedLayout} member={matchedMember} logo={logo} /> : <Image className={`${completedDesignPreviewClassName} block h-auto w-full rounded-sm`} src={mockup.imageUrl} alt={mockup.title} width={completedBusinessCardMockupImageSize.width} height={completedBusinessCardMockupImageSize.height} sizes={completedDesignPreviewImageSizes} unoptimized />}
              actions={<>
                <AppButton className="py-2 text-xs" variant="success" onClick={() => onEdit(entry, matchedMember, completedLayout)} disabled={!mockup.cleanImageUrl || !completedLayout || !matchedMember}>디자인 수정하기</AppButton>
                <AppButton className="py-2 text-xs" variant="primary" onClick={() => onDownloadPdf(entry, matchedMember, completedLayout)} disabled={runningMockupPdfId === mockup.id || !mockup.cleanImageUrl}>
                  {runningMockupPdfId === mockup.id ? "PDF 만드는 중" : pdfRecords[pdfRecordKey] ? "PDF 다운 받기" : "인쇄용 PDF 파일 만들기"}
                </AppButton>
                <AppButton className="py-2 text-xs" variant="danger" onClick={() => onDelete(entry)} disabled={!entry.draft}>삭제하기</AppButton>
              </>}
              notices={<>
