...
export async function POST(request: Request) {
  ...
  const referenceImageDataUrl = isRecord(body) && typeof body.referenceImageDataUrl === "string" ? body.referenceImageDataUrl : undefined;
  ...
  try {
    const response = await createAiBusinessCardJob("mockups", { backgroundEdit: true, cleanImageUrl, editRequest, referenceImageDataUrl }, getAiBusinessCardClientKey(request), JSON.stringify({ backgroundEdit: true, cleanImageUrl, editRequest, referenceImageDataUrl: referenceImageDataUrl ? "attached" : "", requestedAt: Date.now() }));
